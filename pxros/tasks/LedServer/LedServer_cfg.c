...
#define TASK_STACKSIZE          400    // in bytes
#define TASK_INTR_STACKSIZE     32     // in bytes
...
#define TASK_PRIVILIGES PXUser1Privilege
...
{
    {(unsigned int)&MODULE_P13, (unsigned int)(&MODULE_P33 + sizeof(Ifx_P)), WRProtection},
...
{
    // read-only data area - inherits read-only area from Parent task
    .protection[0] =
    {
        .lowerBound = 0,
        .upperBound = 0,
	...
	// data area - task does not use any global data, all data expected on stack
    .protection[1] =
    {
        .lowerBound = -1,
        .upperBound = -1,
...
extern void LedServer_taskFunc(PxTask_t myID, PxMbx_t myMailbox, PxEvents_t myActivationEvents);
...
PxTask_t LedServer_Create(PxPrio_t prio, PxEvents_t events, PxMc_t memClass, PxOpool_t objPool)
{
    ...
    /* Configure Task spec */
    task_Spec.ts_name                       = (const PxChar_t *)"Task LedServer";
    task_Spec.ts_fun                        = LedServer_taskFunc;
